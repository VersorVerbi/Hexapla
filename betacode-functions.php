 ...
 */
function betaString2Unicode(string $str): string
{
    $str = strtoupper($str);
    ...
    foreach ($betaArray as $char) {
        if ($char == '*') { // is capital
            ...
            if (!str_contains($curOutStr, $char)) {
                $curOutStr .= $char;
            } else {
                $curOutStr = resetBeta($checks, $char, $curOutStr, $betaOut);
            }
 ...
 */
function uniString2Betacode(string $str): string
{
    $outString = "";
 ...
 */
function uni2betacode(string $unicodeChar): string {
 ...
 */
function numInRange(int $num, string $range): bool
{
 ...
 */
function beta2unicode(string $betaChunk): string
{
    $rangesForCommonality = [];
    if (str_contains($betaChunk,'*')) {
    ...
    foreach (DIACRITIC_RANGES as $marker => $range) {
        if (str_contains($betaChunk, $marker)) {
 ...
 */
function findRangeCommonality(array $rangeSet, string $sourceStr): int {
    $allValues = [];
